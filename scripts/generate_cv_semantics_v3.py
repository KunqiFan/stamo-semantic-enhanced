"""Generate cross-validated predicted semantics using ALL available features:
delta_z + delta_pooled + action + ee_delta from the manifest.

This version dramatically improves gripper_state and contact_state predictions
by including proprioceptive signals alongside visual latents.
"""
...
def load_all_features(latent_dir: Path, row: dict) -> np.ndarray:
    """Load latent features AND proprioceptive features from manifest."""
    ...
    # Statistics over delta_z
    dz_2d = data["delta_z"].astype(np.float32)
    stats = []
    for r in dz_2d:
        stats.extend([
            r.mean(), r.std(), np.abs(r).max(),
    ...
    # Gripper interaction features
    gripper_is_closing = float(gripper_cmd < 0.3)
    gripper_is_opening = float(gripper_cmd > 0.7)
    low_movement = float(xyz_norm < 0.01)
    grasp_signal = gripper_is_closing * low_movement  # closing + stationary → grasp
    place_signal = gripper_is_opening * low_movement   # opening + stationary → place
...
def main() -> None:
    ...
    x_train = np.stack([load_all_features(args.latent_dir, r) for r in train_rows])
    x_eval = np.stack([load_all_features(args.latent_dir, r) for r in eval_rows])
